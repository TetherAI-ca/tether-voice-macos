...
os.umask(0o077)
folder = Path.home() / "Library/Application Support/Tether Voice/Signing"
...
try:
    if not identity_file.exists():
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="tether-voice-signing-") as temporary:
                temp = Path(temporary)
                ...
                run("/usr/bin/openssl", "req", "-x509", "-newkey", "rsa:2048", "-nodes", "-sha256",
                    ...
                    "-subj", "/CN=Tether Voice Local Development/",
    ...
    try:
        ...
        run("codesign", "--force", "--sign", identity_file.read_text(), "--keychain", str(keychain),
            "--timestamp=none", "--identifier", "ai.tether.voice", sys.argv[1])
    finally:
        run("security", "list-keychains", "-d", "user", "-s", *old_search_list)
